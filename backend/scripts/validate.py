...
def main() -> int:
    ...
    run([sys.executable, "-c", "from fastapi.testclient import TestClient; from backend.main import app; c=TestClient(app); payload={'symbol':'AAPL','side':'BUY','quantity':1,'order_type':'market','bid':99,'ask':101,'last':100}; r=c.post('/api/execution/paper/order',json=payload); assert r.status_code==200 and r.json()['status'] in ['filled','rejected','unfilled']"] )
