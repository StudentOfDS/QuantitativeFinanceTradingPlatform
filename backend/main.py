...
from backend.intelligence import MLEngine
...
def report_full(request: FullReportRequest) -> dict:
    try:
        report = build_research_report(request.rows, request.strategy, request.strategy_params, request.initial_capital, request.transaction_cost_bps, request.slippage_bps, include_ml=request.include_ml)
