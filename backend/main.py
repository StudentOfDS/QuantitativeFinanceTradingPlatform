...
from backend.intelligence import MLEngine, rl_action, rl_backtest
from backend.time_series import time_series_context
from backend.execution import ExecutionRouter, GenericRestBroker, KillSwitch, MarketSnapshot, OrderRequest, PaperBroker, RiskManager, decision_to_orders
from backend.storage import initialize_storage, list_recent_reports, record_audit_event, record_backtest_run, record_model_validation, record_report_run

app = FastAPI(title=settings.app_name)

initialize_storage()
kill_switch = KillSwitch()
paper_broker = PaperBroker(commission_bps=1.0, slippage_bps=0.0)
live_broker = GenericRestBroker({'TRADING_MODE': settings.trading_mode, 'ENABLE_LIVE_TRADING': settings.enable_live_trading, 'DRY_RUN': settings.dry_run, 'BROKER_API_KEY': settings.broker_api_key, 'BROKER_API_SECRET': settings.broker_api_secret, 'BROKER_ENDPOINT': settings.broker_endpoint})
risk_manager = RiskManager(allowed_symbols={'AAPL','MSFT','SPY','QQQ'}, max_qty=100000, max_notional=settings.max_order_notional)
exec_router = ExecutionRouter(paper_broker, live_broker, risk_manager, kill_switch)
app.add_middleware(
    CORSMiddleware,
...
def report_full(request: FullReportRequest) -> dict:
    try:
        report = build_research_report(request.rows, request.strategy, request.strategy_params, request.initial_capital, request.transaction_cost_bps, request.slippage_bps, include_ml=request.include_ml, include_rl=request.include_rl, include_time_series=request.include_time_series)
...
def ml_validate(request: MLRequest) -> dict:
    try:
        df = InputAdapter.from_manual_rows(request.rows)
        normalized, _ = DataEngine.normalize_market_data(df)
        result = MLEngine.validate(normalized)
        result['model_validation_id'] = record_model_validation(result)
        return result
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
...
def ml_predict(request: MLRequest) -> dict:
    try:
        df = InputAdapter.from_manual_rows(request.rows)
        normalized, _ = DataEngine.normalize_market_data(df)
        return MLEngine.predict_latest(normalized)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
...
def rl_action_endpoint(request: RLRequest) -> dict:
    try:
        df = InputAdapter.from_manual_rows(request.rows)
        normalized, _ = DataEngine.normalize_market_data(df)
        return rl_action(normalized, request.current_exposure, settings.global_random_seed)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc


@app.post('/api/rl/backtest')
def rl_backtest_endpoint(request: RLRequest) -> dict:
    try:
        df = InputAdapter.from_manual_rows(request.rows)
        normalized, _ = DataEngine.normalize_market_data(df)
        return rl_backtest(normalized, request.transaction_cost_bps, settings.global_random_seed)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
...
@app.post('/api/time-series/context')
def time_series_context_endpoint(request: TimeSeriesRequest) -> dict:
    try:
        df = InputAdapter.from_manual_rows(request.rows)
        normalized, _ = DataEngine.normalize_market_data(df)
        return time_series_context(normalized)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
...
@app.post('/api/execution/paper/order')
def execution_paper_order(request: ExecutionOrderRequest) -> dict:
    order = OrderRequest(request.symbol, request.side, request.quantity, request.order_type, request.limit_price)
    snap = MarketSnapshot(request.symbol, request.bid, request.ask, request.last)
    report, risk = exec_router.route_paper_order(order, snap)
    from backend.storage import record_execution_bundle
    rid = record_execution_bundle(order.__dict__, report.__dict__, {k:v.__dict__ for k,v in paper_broker.portfolio.positions.items()}, {'cash': str(paper_broker.portfolio.cash)}, {'fee': report.fee}, {'risk': risk.reason, 'status': report.status})
    out = report.__dict__.copy(); out['execution_report_id']=rid
    return out

@app.post('/api/execution/paper/decision')
def execution_paper_decision(request: ExecutionDecisionRequest) -> dict:
    orders = decision_to_orders(request.action, request.symbol, request.quantity, request.second_symbol)
    reports=[]
    for o in orders:
        snap = MarketSnapshot(o.symbol, request.bid, request.ask, request.last)
        rep,_ = exec_router.route_paper_order(o, snap)
        reports.append(rep.__dict__)
    return {'reports': reports}

@app.get('/api/execution/paper/portfolio')
def execution_paper_portfolio() -> dict:
    return {'cash': float(paper_broker.portfolio.cash), 'positions': {k:{'quantity': float(v.quantity),'avg_entry_price': float(v.avg_entry_price),'realized_pnl': float(v.realized_pnl)} for k,v in paper_broker.portfolio.positions.items()}, 'kill_switch_active': kill_switch.active}

@app.get('/api/execution/audit/recent')
def execution_audit_recent(limit: int = 20) -> dict:
    from backend.storage import list_recent_execution_audits
    return {'items': list_recent_execution_audits(limit)}

@app.post('/api/execution/killswitch')
def execution_killswitch(active: bool) -> dict:
    kill_switch.active = active
    return {'kill_switch_active': kill_switch.active}

@app.post('/api/execution/live/order')
def execution_live_order(request: ExecutionOrderRequest) -> dict:
    order = OrderRequest(request.symbol, request.side, request.quantity, request.order_type, request.limit_price)
    snap = MarketSnapshot(request.symbol, request.bid, request.ask, request.last)
    report,_ = exec_router.route_live_order(order, snap)
